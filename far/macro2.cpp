...
struct TMacroKeywords MKeywords[] ={
  ...
  {2,  "APanel.Current",     MCODE_C_APANEL_CURRENT,0},
  {2,  "PPanel.Current",     MCODE_C_PPANEL_CURRENT,0},
  {2,  "APanel.SelСount",    MCODE_C_APANEL_SELCOUNT,0},
  {2,  "PPanel.SelСount",    MCODE_C_PPANEL_SELCOUNT,0},
  {2,  "APanel.Path",        MCODE_C_APANEL_PATH,0},
  {2,  "PPanel.Path",        MCODE_C_PPANEL_PATH,0},


  {2,  "Editor.State",       KEY_MACRO_EDITORSTATE,0},
  ...
  switch(MKeywords[I].Type)
  {
    ...
    case 2:
    {
      ...
      switch(CheckCode)
      {
        ...
        //- AN -----------------------------------------------------
        case MCODE_C_APANEL_CURRENT: // APanel.Current
        case MCODE_C_PPANEL_CURRENT: // PPanel.Current
        {
          Panel *SelPanel = CheckCode == MCODE_C_APANEL_CURRENT ? ActivePanel : PassivePanel;
          if ( SelPanel != NULL )
          {
            SelPanel->GetFileName(FileName,SelPanel->GetCurrentPos(),FileAttr);
            if ( FileAttr != -1 )
        ...
        case MCODE_C_APANEL_SELCOUNT: // APanel.SelCount
        case MCODE_C_PPANEL_SELCOUNT: // PPanel.SelCount
        {
          Panel *SelPanel = CheckCode == MCODE_C_APANEL_SELCOUNT ? ActivePanel : PassivePanel;
          if ( SelPanel != NULL )
            Cond = (long)SelPanel->GetRealSelCount();
          break;
        }

        case MCODE_C_APANEL_PATH: // APanel.Path
        case MCODE_C_PPANEL_PATH: // PPanel.Path
        {
          Panel *SelPanel = CheckCode == MCODE_C_APANEL_PATH ? ActivePanel : PassivePanel;
          if ( SelPanel != NULL )
          {
        ...
        case KEY_MACRO_EDITORSTATE: // Editor.State
        {
          if(CtrlObject->Macro.GetMode()==MACRO_EDITOR &&
             CtrlObject->Plugins.CurEditor &&
             CtrlObject->Plugins.CurEditor->IsVisible())
            Cond=(long)CtrlObject->Plugins.CurEditor->ProcessKey(KEY_MACRO_EDITORSTATE);
          break;
        }
      }
    }
...
static void printKeyValue(DWORD* k, int& i)
{
  ...
  else if ( k[i] == KEY_MACRO_DATE )           sprint(ii, "$date ''");
  else if ( k[i] == KEY_MACRO_PLAINTEXT )      sprint(ii, "$text ''");
