...
void FileList::ShowTotalSize(struct OpenPluginInfo &Info)
{
  ...
  *FreeSize=0;
  ...
  if (Opt.ShowPanelTotals)
    ...
    {
      sprintf(TotalStr," %s (%d) อออ %s ",FormSize,TotalFileCount,FreeSize);
      if (strlen(TotalStr)>X2-X1-1)
      {
        InsertCommas(FreeDiskSize>>20,FreeSize);
        InsertCommas(TotalFileSize>>20,FormSize);
        sprintf(TotalStr," %s %s (%d) อออ %s %s ",FormSize,MSG(MListMb),TotalFileCount,FreeSize,MSG(MListMb));
      }
    }
  else
  ...
  char *FirstBox=strchr(TotalStr,'อ');
  int BoxPos=(FirstBox==NULL) ? -1:FirstBox-TotalStr;
  int BoxLength=0;
  if (BoxPos!=-1)
    for (int I=0;TotalStr[BoxPos+I]=='อ';I++)
      BoxLength++;
