...
if exist custom.vc.release.IA64.bat call custom.vc.release.IA64.bat

rem generate/update lng files and lang.hpp
tools\lng.generator.exe -nc -i lang.ini farlang.templ

rem rebuild dependencies
tools\gawk -f .\scripts\mkdep.awk -v out=Release.IA64.vc mkdep.list > far.release.dep

nmake %NMAKE_OPTS% /f "FAR.mak" CFG="far - WinIA64 %CFG_0%" | tools\tee !Error_vc_IA64_%CFG_0%
