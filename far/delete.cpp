...
int ShellRemoveFile(const char *Name,const char *ShortName,int Wipe)
{
	...
	while (1)
	{
		...
		else if (WinVer.dwPlatformId==VER_PLATFORM_WIN32_NT && WinVer.dwMajorVersion<4 || !Opt.DeleteToRecycleBin)
		{
			/*
...
int ERemoveDirectory(const char *Name,const char *ShortName,int Wipe)
{
	...
	while (1)
	{
		...
		else if (FAR_RemoveDirectory(Name) || (_localLastError != ERROR_ACCESS_DENIED && FAR_RemoveDirectory(ShortName)))
			break;
