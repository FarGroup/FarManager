...
Modify:
  ...
  02.04.2005 AY
    ...
    - Если в FileSizeToStr() при COLUMN_FLOATSIZE остаток деления был меньше
      чем 10% то размер после точки выводился не правильно.
  02.04.2005 SVS
    ! уберем пробел, так "интереснее"
  02.04.2005 AY
    + Обработка COLUMN_FLOATSIZE в FileSizeToStr()
  23.12.2004 WARP
    ! Все-таки нехорошо делать strcpy для пересекающихся строк.
  28.10.2004 SVS
