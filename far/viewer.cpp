...
void Viewer::ShowPage (int nMode)
{
  ...
  if ( nMode != SHOW_HEX )
  {
    for (I=0,Y=ViewY1;Y<=Y2;Y++,I++)
    {
      int StrLength = wcslen(Strings[I]->lpData);
...
void Viewer::ShowHex()
{
  ...
  for (EndFile=0,Y=ViewY1;Y<=Y2;Y++)
  {
    ...
    if ( (SelEnd <= SelStart) && bSelStartFound )
       SelEnd = wcslen (OutStr)-2;

    wcscat(OutStr,L" ");

    wcscat(OutStr,TextStr);
    if (wcslen(OutStr)>HexLeftPos)
...
void Viewer::ReadString (ViewerString *pString, int MaxSize, int StrSize)
{
  ...
  if ( bSelStartFound )
  {
    if ( pString->nSelStart > wcslen (pString->lpData) )
      bSelStartFound = false;
...
void Viewer::Search(int Next,int FirstChar)
{
  ...
  */
  ...
  /* 07 */ DI_CHECKBOX,40,5,0,0,0,0,0,0,(const wchar_t *)MViewSearchCase,
  /* 08 */ DI_CHECKBOX,40,6,0,0,0,0,0,0,(const wchar_t *)MViewSearchWholeWords,
  /* 09 */ DI_CHECKBOX,40,7,0,0,0,0,0,0,(const wchar_t *)MViewSearchReverse,
  /* 10 */ DI_TEXT,3,8,0,0,0,0,DIF_BOXCOLOR|DIF_SEPARATOR,0,L"",
  ...
  */
  ...
  if ((SearchLength=strSearchStr.GetLength ())==0)
    return;
...
int Viewer::vread(wchar_t *Buf,int Count,FILE *SrcFile)
{
  ...
  if (VM.Unicode)
  {
    ...
    int ReadSize=fread(TmpBuf,1,Count*2,SrcFile);
    TmpBuf[ReadSize]=0;
    /* $ 20.10.2000 tran
       обратный порядок байтов */
    TmpBuf[ReadSize+1]=0;
  ...
  else
  {
    ...
    int ReadSize=fread(TmpBuf,1,Count,SrcFile);
...
void Viewer::GoTo(int ShowDlg,__int64 Offset, DWORD Flags)
{
  ...
  static struct DialogDataEx GoToDlgData[]=
  {
    ...
    /* 5 */ DI_RADIOBUTTON,5,6,0,0,0,0,0,0,(const wchar_t *)MGoToDecimal,
  };
...
//  LastSelPos=SelectPos+((Flags&0x2) ? -1:1);
  ...
  */
    ...
    SelectPosOffSet=(VM.Unicode && (FirstWord==0x0FFFE || FirstWord==0x0FEFF)
           && (MatchPos+SelectSize<=ObjWidth && MatchPos<wcslen(Strings[0]->lpData)))?1:0;
