...
*/
    ...
    */
    static struct DialogData FindAskDlgData[]=
    {
      ...
      /* 05 */DI_EDIT,5,6,36,5,0,(DWORD_PTR)TextHistoryName,DIF_HISTORY,0,"",
      /* 06 */DI_FIXEDIT,5,6,36,5,0,(DWORD_PTR)HexMask,DIF_MASKEDIT,0,"",
    ...
    if (RealReadSize==sizeof(Buf))
    {
      ...
      */
      ...
      if ((UseAllTables || UnicodeSearch)&& !SearchHex)
        NewPos=ftell64(SrcFile)-2i64*(__int64)(Length+1);
      else
...
/* $ 12.05.2004 KM
   Получим размер, в котором будет производиться поиск,
