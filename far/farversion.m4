m4_divert(`-1')

# Uncomment to create a special version
#m4_define(`SPECIAL_VERSION', epic feature)

m4_define(VERSION_MAJOR, 3)
m4_define(VERSION_MINOR, 0)
m4_define(VERSION_REVISION, 0)
m4_define(VERSION_BUILD, m4_patsubst(m4_include(`vbuild.m4'),`
',`'))

m4_define(`VERSION_STAGE', m4_ifdef(`SPECIAL_VERSION', VS_SPECIAL, m4_ifelse(BUILD_TYPE, `', VS_PRIVATE, BUILD_TYPE)))

m4_define(BUILD_SPECIAL, m4_ifelse(VERSION_STAGE, VS_SPECIAL, 1, 0))
m4_define(BUILD_PRIVATE, m4_ifelse(VERSION_STAGE, VS_PRIVATE, 1, 0))
m4_define(BUILD_PRERELEASE, m4_ifelse(
		VERSION_STAGE, VS_ALPHA,   1,
		VERSION_STAGE, VS_PRIVATE, 1,
		VERSION_STAGE, VS_SPECIAL, 1,
		0))

m4_include(`tools.m4')

m4_define(BUILD_DATE, m4_esyscmd(CMDAWK -f ./scripts/gendate.awk))
m4_define(BUILD_YEAR, m4_substr(BUILD_DATE,6,4))
m4_define(BUILD_MONTH, m4_substr(BUILD_DATE,3,2))
m4_define(BUILD_DAY, m4_substr(BUILD_DATE,0,2))
m4_define(COPYRIGHTYEAR, BUILD_YEAR)

m4_define(FULLVERSION, VERSION_MAJOR.VERSION_MINOR.VERSION_REVISION.VERSION_BUILD`'m4_ifelse(
		VERSION_STAGE, VS_SPECIAL, ` (Special: 'SPECIAL_VERSION`)',
		VERSION_STAGE, VS_PRIVATE, ` (Private)',
		VERSION_STAGE, VS_ALPHA,   ` (Alpha)',
		VERSION_STAGE, VS_BETA,    ` (Beta)',
		VERSION_STAGE, VS_RC,      ` (RC)',
		VERSION_STAGE, VS_RELEASE, `',
		` (Unknown)') m4_ifelse(FARBIT, 64, x64, x86))

m4_divert(0)m4_dnl
