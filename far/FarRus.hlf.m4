...
Windows 95/98/Me/NT/2000/XP/2003/Vista/2008, которая обеспечивает обработку
...
поведение регулируется в ~настройках~@InterfSettings@.
   ...
   В Windows 2000/XP/2003/Vista/2008 FAR Manager умеет обрабатывать
~колесо мыши~@MsWheel@.
...
$ #Мышь: поддержка колеса#
    В Windows 2000/XP/2003/Vista/2008 FAR Manager умеет обрабатывать колесо мыши:
...
системой NTFS.
    ...
    В Windows 2000/XP/2003/Vista/2008 для ~символических связей~@HardSymLink@
так же отображается путь к исходным папкам.
...
параметров.
   ...
   #Размер#          Минимальный и максимальный размер файла в #байтах#,
   ...
   #Атрибуты#        Атрибуты включения и атрибуты исключения.
                   ...
                   #Разреженный# и #Временный# используются только на
                   дисках с файловой системой NTFS. При этом атрибут
                   #Зашифрованный# используется только в
...
$ #Системные параметры#
  ...
  #Удалять символические#   Перед удалением каталога в Корзину,
  #связи#                   просканировать и удалить линки на
                          подкаталоги.
...
является ~панелью файлов~@FilePanel@, ее тип будет изменен на панель файлов.
    ...
    - извлечения дисков из CD-ROM и сменных приводов. В Windows
      NT/2000/XP/2003/Vista/2008 для операции извлечения диска из ZIP-приводов
...
используется только в Windows 2000/XP/2003/Vista/2008.
...
Причем, атрибут "Зашифрованный" используется только в Windows
2000/XP/2003/Vista/2008. Атрибуты "Сжатый" и "Зашифрованный" являются
взаимоисключающими, т.е. можно выставить только один из них.

    Для ~символических связей~@HardSymLink@ в диалоге отображается информация
об оригинальной папке (только в NTFS/Windows 2000/XP/2003/Vista/2008). Если
информация об оригинальной папке недоступна (в частности, для символических
связей в сетевых каталогах), то выводится сообщение "#(нет данных)#".
...
NTFS/Windows 2000/XP/2003/Vista/2008) дата и время не выставляются.
...
сгенерированными фильтрами (маски файлов), осуществляется следующими клавишами:
   ...
   #Ctrl-F#              Изменить состояние разрешения на фильтрацию
                       папок. Если фильтрация папок разрешена, то в
                       заголовке меню будет отображена строка "D?",
                       а если запрещена (папки будут показаны
                       обязательно), то - "D+".
   ...
   #Shift-<Gray Minus>#  Снять все пометки в значение по умолчанию
                       (выставит состояние разрешения на фильтрацию
                       папок в запрещенное состояние, остальные
                       пометки будут сброшены).
...
связи можно только на том же диске, на котором расположен исходный файл.
    ...
    NTFS начиная с версии 5.0 (Windows 2000/XP/2003/Vista/2008) поддерживает
символические связи (SymLink). Символические связи папок в Windows
2000/XP/2003/Vista/2008 известны как "directory junctions" - технология,
позволяющая отображать любые локальные папки на любые другие локальные папки.
Например, если папка D:\\SYMLINK указывает на C:\\WINNT\\SYSTEM32 в качестве
своей цели, то программа, обращающаяся к D:\\SYMLINK\\DRIVERS, будет на самом
деле обращаться к C:\\WINNT\\SYSTEM32\\DRIVERS.

    FAR умеет создавать символические связи только на локальных дисках,
файловая система которых поддерживает эту возможность (Windows
2000/XP/2003/Vista/2008/NTFS 5.0). В отличие от жестких связей, символические
связи не обязательно должны указывать на тот же диск.
...
$ #Ошибка: копирование/перенос "в самого себя"#
    ...
    Данная ошибка также возникает в Windows 2000/XP/2003/Vista/2008, если
имеются две папки, одна из которых является ~символической связью~@HardSymLink@
на другую.
...
потоки. В частности, потоки использует Windows Explorer в Windows
2000/XP/2003/Vista/2008 для хранения дополнительных свойств файла. Файловые
системы FAT/FAT32 не поддерживают потоки.
