...
$ #FAR: параметры командной строки#
  ...
  #/8#    Принудительно отключает юникодные возможности консоли.
        Опция работает только под Windows NT 4 и выше.
...
$ #Команды управления панелями  #
 ...
 #Команды файловой панели#
  ...
  1. Если включена опция "Разрешить обратную сортировку" в диалоге
     ~Настроек панели~@PanelSettings@ , то повторное нажатие одной и той же
     клавиши сортировки файлов приводит к смене направления
     сортировки с возрастающей на убывающую и наоборот;

  2. Комбинации Alt-Left and Alt-Right, используемые для прокрутки
     длинных имен и описаний, работают только с клавишами Left
     и Right, расположенными не на numpad. Так происходит потому,
     что при нажатой Alt клавиши управления курсором на numpad
     используются для ввода символов через их десятичные коды.

  3. Комбинация клавиш #Ctrl-Alt-Ins# работает по следующим правилам:
     * для сетевых дисков - сетевое (UNC) имя файлового объекта;
     * для локальных дисков - полное имя с учетом ~символических~@HardSymLink@
       связей.

  4. Комбинации клавиш #Alt-Shift-Ins# и #Ctrl-Alt-Ins# обрабатывают
...
соответствии с введенной в строке файловой маской.
...
$ #Командная строка#
  ...
  1. Если командная строка пуста, #Ctrl-Ins# будет копировать
     имена выбранных в панели файлов в Буфер Обмена так же,
     как и #Ctrl-Shift-Ins# (см. ~Команды управления панелями~@PanelCmd@);

  2. #Ctrl-End#, нажатая в конце командной строки, заменяет ее
     текущее содержимое командой из ~истории команд~@History@, начинающейся
     с уже введенных букв, если такая команда существует.
     Чтобы перейти к следующей такой команде, вы можете нажать
     #Ctrl-End# повторно.

  3. Большинство из описанных выше команд действительно для всех
     строк редактирования, включая строки в диалогах и встроенный
     редактор.

  4. #Alt-Shift-Left#, #Alt-Shift-Right#, #Alt-Shift-Home# и #Alt-Shift-End#
     выделяют блок в командной строке независимо от состояния
     панелей.

  5. Комбинации клавиш, позволяющие получить сетевое (UNC) имя
     файлового объекта, работают по следующим правилам:
     * для сетевых дисков - сетевое (UNC) имя файлового объекта;
     * для локальных дисков - полное имя с учетом ~символических~@HardSymLink@
       связей.
...
осуществляется через него, иначе - средствами FAR.
  ...
  Создание ~связей~@HardSymLink@ файлов и папок (только на NTFS)             #Alt-F6#

    Используя жесткие связи файлов, вы можете иметь несколько различных имен
...
переключает экран с 25 линий на 50 и обратно. Подробнее см. TechInfo##38.
  ...
  Редактировать новый файл                                  #Shift-F4#
    ...
    При создании нового файла используется кодировка, которая была выставлена в
последнем открытом редакторе. Если редактор открывается впервые в текущей
сессии FAR, используется кодировка DOS.
...
#Shift-Enter# на папке с именем "#..#" открывает в Проводнике текущую папку.
  ...
  Смена папки, вход в архив (также в SFX архив)            #Ctrl-PgDn#

    Если курсор указывает на папку, то производится смена текущей папки на
указываемый. Если курсор указывает на файл, то в зависимости от типа файла или
...
клавиш:
    ...
    1. В зависимости от настроек в меню ~Системные параметры~@SystemSettings@
...
не используя Корзину.
    ...
    2. В процессе уничтожения, файл перезаписывается нулями (можно задать
...
переменной окружения %PATHEXT%, начиная с последовательности
".BAT;.CMD;.EXE;.COM;"):
...
непустой ~командной строки~@CmdLineCmd@ будет исполнено ее содержимое. Это
поведение регулируется в ~настройках~@InterfSettings@.

   В Windows 2000/XP FAR Manager умеет обрабатывать ~колесо мыши~@MsWheel@.
...
$ #Мышь: поддержка колеса#
   ...
   #Панели#          Прокрутка списка при неизменной позиции курсора
                   ...
                   нажатию #Enter# (регулируется в ~настройках~@InterfSettings@).
...
меню #Команды внешних модулей#, активизируемого с помощью #F11# или
...
Это меню доступно из файловых панелей и (только по F11) из встроенной программы
просмотра и редактора. При вызове из программы просмотра и редактора будут
показаны не все модули, а только те, которые специально созданы для работы в
этом режиме.
...
горячие клавиши для пунктов этого меню, что упрощает их последующий вызов с
помощью ~клавиатурных макрокоманд~@KeyMacro@.
...
связей файлов тоже применимы только к разделам NTFS. Некоторые файловые системы
могут не поддерживать время создания и время доступа к файлу.
...
$ #Панель информации#
 ...
 - имя и тип #текущего диска#, тип его файловой системы,
сетевое имя, общий и свободный размеры, метку тома и серийный номер;

 - уровень загрузки #памяти# (100% означает, что использована вся память)
общий и свободный размеры физической и виртуальной памяти;
...
~встроенной программы просмотра~@Viewer@ (поиск, выбор кодировки и т.д.) для
...
определения типа привода работает только под NT-based системах и требует или
административных прав, или разрешения доступа к локальным СD-приводам с помощью
политик (для этого надо запустить из командной строки утилиту #secpol.msc# и
установить настройку '#Локальные политики/Параметры безопасности/Устройства:#
#Разрешить доступ к дисководам компакт-дисков только локальным пользователям#' в
значение '#Разрешить#').
...
системой NTFS.
    ...
    В Windows 2000/XP для ~символических связей~@HardSymLink@ так же
отображается путь к исходным папкам.

    Для папок общие размеры могут не соответствовать реальным значениям:

    1. Если среди вложенных папок имеются символические связи и включена опция
"Сканировать символические связи" (диалог настройки
~системных параметров~@SystemSettings@).

    2. Если внутри папки есть несколько жестких связей на один и тот же файл.
...
$ #Меню команд#
   ...
   #файлов#               Дополнительная информация об этой команде
                        содержится в теме ~Фильтр панели файлов~@Filter@.
...
$ #Меню параметров#
   ...
   #Раскраска файлов#     Редактирование ~раскраски файлов~@Highlight@.
...
файловыми системами, поддерживаемыми с помощью ~внешних модулей~@Plugins@.
    ...
    Примечание: если маска, введенная пользователем, не содержит
                символов '?', '*' и '.', то FAR к введенному
                пользователем значению добавляет символ '*' и
                производит поиск по измененной маске.
...
параметры #Учитывать регистр#, #Только целые слова#, #Используя таблицу символов#
и #Искать папки# блокируются и их значения на процесс поиска не
влияют.

    Выпадающий список #Используя таблицу символов# позволяет выбрать конкретную
таблицу символов, используемую для поиска текста или заставляет FAR
использовать все доступные ему таблицы, если выбран пункт #Все таблицы#
#символов#, для поиска текста в файлах с различной кодировкой.
...
и найденные папки.
    ...
    Параметр #Искать в символических связях# позволяет искать файлы в
 ~символических связях~@HardSymLink@ наравне с обычными подкаталогами.

     Примечание: если при помощи символических связей в файловой
                 системе образованы рекурсивные кольца (например,
                 символическая связь указывает на один из
                 собственных родительских каталогов), поиск будет
                 идти до достижения максимально допустимой длины
                 полного пути файлов.
    ...
    - в отмеченных папках (в текущей версии FAR поиск в каталогах,
      которые являются ~символическими связями~@HardSymLink@, не производится).
...
#Использовать фильтр#. Настройка условий ~фильтра~@OpFilter@ осуществляется кнопкой #Фильтр#.

    С помощью кнопки #Дополнительно# вызывается диалог настройки ~дополнительных параметров поиска~@FindFileAdvanced@,
которые используются для настройки расширенного набора свойств поиска. Для того
чтобы быстро включить или отключить эти параметры используется переключатель
...
@OpFilter
$ #Фильтр операций#
    Фильтр операций служит для обработки определённых групп файлов по заданным
...
параметров, и может включать в себя:

     - одну или несколько ~масок файлов~@FileMasks@;

     - минимальный и максимальный размер файла включительно;

     - начальную дату и дату окончания действия фильтра,
       указывая дату последней модификации, дату создания
       файла или дату последнего доступа к файлу;

     - атрибуты включения и атрибуты исключения.

    Файл попадает в условия фильтра, если:

     - включен анализ масок файла и его имя соответствует
       хотя бы одной маске. Если анализ масок отключен, то
       имя файла не имеет значения;

     - включен анализ размера файла и он попадает в введённый
       диапазон. Если в качестве одной или обеих границ
       диапазона ничего не введено (пустая строка), тогда размер
       по этой границе не ограничивается;

     - включен анализ даты файла и она попадает в введённый
       диапазон и соответствует установленному типу:
       дате последней модификации, дате создания или дате
       последнего доступа к файлу. Если одно или оба поля
       с датами пусто, тогда дата по этому полю не
       ограничивается;

     - включен анализ атрибутов файла и он имеет все атрибуты
    ...
    Атрибуты #Сжатый#, #Зашифрованный#, #Неиндексируемый#, #Разреженный# и
#Временный# используются только на дисках с файловой системой NTFS. При этом
атрибут #Зашифрованный# используется только в Windows 2000/XP.
...
указать "-" (минус), а поле "Метка" оставить пустым. Удалить такой разделитель
можно только через #Alt-F4#.
...
цифры, буквы и функциональные клавиши (F1..F12). Если использованы #F1# и #F4#,
...
~метасимволы~@MetaSymbols@.
  ...
  1. Если для данного файла отсутствуют ассоциированные команды
...
ассоциации Windows для запуска этого типа файлов.
  ...
  2. ~Команды~@OSCommands@ операционной системы "IF EXIST" и "IF DEFINED"
...
метасимволы:
    ...
    #!=\\#      Текущий путь с учетом ~символических~@HardSymLink@ связей.
    #!=/#      Короткое имя текущего пути с учетом ~символических~@HardSymLink@
             связей.
  ...
  Примечания:
    ...
    1. FAR при обработке метасимволов подставляется только то, что они означают
...
файлов с полным путем в ANSI кодировке; имена файлов, содержащие пробелы будут
заключены в кавычки.

    3. Метасимволы !@@! и !$! в меню выбора (когда задано несколько ассоциаций)
показываются как есть, преобразование происходит в момент исполнения команды.

    4. Префиксы "!##" и "!^" работают как переключатели для ассоциаций.
...
Например:
    ...
    5. Если нужно передать программе имя с завершающей обратной чертой,
...
@SystemSettings
$ #Системные параметры#
  #Снимать атрибут R/O#     Снимать атрибут "Только для чтения"
  #с CD файлов#             с файлов, копируемых с CD.

  #Удалять в Корзину#       Разрешает удаление файлов с использованием
                          Корзины (Recycle Bin). Операция удаления в
                          Корзину осуществляется только для локальных
                          жестких дисков.

  #Использовать системную#  Использовать функции копирования файлов,
  #функцию копирования#     предоставляемые операционной системой,
                          вместо внутренней реализации копирования
                          файлов. Это может быть полезно на NTFS,
                          так как системная функция (CopyFileEx)
                          выполняет более эффективное распределение
                          дискового пространства и копирует
                          расширенные атрибуты файлов. В то же
                          время, при включенной опции будет
                          недоступна возможность разрезать файлы на
                          части при ~копировании~@CopyFiles@ или перемещении.

  #Копировать открытые#     Позволяет копировать файлы, которые
  #для записи файлы#        открыты для записи другими программами.
                          Этот режим удобен, когда нужно скопировать
                          открытый в течение долгого времени файл,
                          но он может быть опасным, если этот файл
                          модифицируется одновременно с копированием.

  #Сканировать#             Обрабатывать ~символические связи~@HardSymLink@ наравне
  #символические связи#     с обычными подкаталогами при построении
                          дерева каталогов, определении суммарного
                          размера файлов в подкаталогах.

  Примечание: если при помощи символических связей в файловой системе
              образованы рекурсивные кольца (например, символическая
              связь указывает на один из собственных родительских
              каталогов), сканирование будет идти до достижения
              максимально допустимой длины полного пути файлов.

  #Создавать папки#         Если имя новой папки содержит только
  #заглавными буквами#      строчные буквы и установлена эта опция,
                          то папка будет создана заглавными буквами.

  #Время бездействия#       Завершает работу FAR, если в течение
                          указанного интервала не было нажатий клавиш
                          мыши или клавиатуры, FAR ожидал ввода
                          из командной строки и отсутствовали фоновые
                          экраны редактирования или просмотра.

  #Сохранять историю#       Вызывает сохранение ~истории команд~@History@ перед
  #команд#                  завершением и ее восстановление после
                          запуска FAR.

  #Сохранять историю#       Вызывает сохранение ~истории папок~@HistoryFolders@ перед
  #папок#                   завершением и ее восстановление после
                          запуска FAR. Для просмотра содержимого
                          истории папок нажмите #Alt-F12#.

  #Сохранять историю#       Вызывает сохранение ~истории просмотра~@HistoryViews@
  #просмотра и редактора#   и редактора перед завершением и ее
                          восстановление после запуска FAR.
                          Для просмотра входящих в этот список файлов
                          нажмите #Alt-F11#.

  #Использовать#            Если это опция включена, то при нажатии
  #стандартные типы#        Enter на файле, тип которого известен
  #файлов#                  Windows и отсутствует в ~ассоциациях файлов~@FileAssoc@
                          FAR, будет запущена программа Windows,
                          предназначенная для обработки этого типа
                          файлов.

  #Автоматически#           При выборе CDROM из меню ~выбора дисков~@DriveDlg@
  #монтировать CDROM#       FAR задвинет открытый лоток CD-привода.
                          Отключите эту опцию, если автоматическое
                          монтирование CD-приводов работает
                          некорректно (это может происходить из-за
                          ошибок в драйверах некоторых CD-приводов)

  #Путь к#                  Укажите полный путь, где FAR будет искать
  #персональным плагинам#   "персональные" плагины в дополнение
                          к "основным". Можно указать несколько путей
                          поиска, разделенных символом ';'. При
                          указании пути поиска можно использовать
                          переменные среды.
                          Персональные плагины не будут загружаться,
                          если в параметрах ~командной строки~@CmdLine@ указан
                          один из ключей /p или /co.

  #Автозапись#              Если эта опция включена, FAR будет
  #конфигурации#            автоматически сохранять конфигурацию.
                          Также будут сохраняться текущие папки
                          обеих панелей.
...
$ #Настройка интерфейса#
  ...
  #Часы#                    Показывать часы при редактировании
  #при редактировании#      и просмотре файлов.
  #и просмотре#

  #Мышь#                    Использовать мышь.

  #В панелях средняя#       Нажатие средней кнопки мыши в панелях
  #кнопка равна Enter#      эквивалентно нажатию на клавишу #Enter#.

  #Показывать линейку#      Показывать назначения функциональных
  #клавиш#                  клавиш в нижней строке экрана. Эта опция
                          также может переключаться по #Ctrl-B#.

  #Всегда показывать меню#  Показывать меню вверху экрана, даже
                          когда оно неактивно.

  #Сохранение экрана#       Запуск программы сохранения экрана
                          после заданного в минутах интервала
                          бездействия. Хранитель экрана так же
                          включается, если подвести указатель
                          мыши в правый верхний угол окна
                          FAR Manager (при выбранной опции).

  #Установить формат#       Эта опция позволяет изменить
  #командной строки#        ~формат приглашения~@CommandPrompt@ командной строки.

  #Использовать правый#     Установите эту опцию, если вы испытываете
  #Alt как AltGr#           проблемы с использованием комбинаций
                          #правый Alt# для ввода символов
                          в Windows 9x, или отключите ее, если вы
                          предпочитаете использовать #правый Alt#
                          для быстрого поиска.

                          Эта опция имеет значение только при работе
                          в Windows 9x, и игнорируется под
                          Windows NT.

  #Показывать общий#        Показывать общий индикатор во время
  #индикатор копирования#   выполнения операции копирования. Это
                          может потребовать дополнительного времени
                          перед началом копирования для подсчета
                          общего размера файлов.

  #Показывать информацию#   Выводить информации о средней скорости
  #о времени копирования#   копирования, времени копирования и
                          примерном времени до конца операции в
                          диалоге копирования.
  ...
  #Использовать Ctrl-PgUp#  Комбинация клавиш #Ctrl-PgUp# в корневой
  #для выбора диска#        папке:
                          - для локальных дисков вызовет меню
                            выбора дисков;
                          - для сетевых дисков вызовет плагин
...
$ #Настройка диалогов#
  ...
  #в строках ввода#         курсора в строках ввода в диалогах и
                          в командной строке.
...
~Настроек интерфейса~@InterfSettings@ необходимо ввести нужную
...
и путь ("C:\>").
   ...
   1. Приглашение формата #[%COMPUTERNAME%]$S$P$G# будет
      содержать имя компьютера, текущий диск и путь
      (переменная окружения %COMPUTERNAME% должна быть определена)

   2. Приглашение формата #[$T$H$H$H]$S$P$G# перед текущем путем
...
$ #Программа просмотра: клавиши управления#
   Команды программы просмотра
    ...
    #F8#                 Переключить режим просмотра текста DOS/Windows
    #Shift-F8#           Выбор пользовательской таблицы символов
                       (см. примечания)
    ...
    1. Пользовательские таблицы символов находятся в папке
       "Addons\\Tables" папки FAR в виде .reg файлов.
       Перед использованием любой из таблиц ее нужно установить:
       для этого просто нажмите #Shift-Enter# на соответствующем файле.

    2. Для вызова диалога поиска вы также можете просто
       начать вводить предназначенный для поиска текст.

    3. Файл открывается в программе просмотра с разрешением на
    ...
    4. В текущей версии FAR существует ограничение на максимальное
...
опции:
    ...
    #Искать текст#
      ...
      В этом режиме доступны следующие опции:
        ...
        #Учитывать регистр#   - при поиске будет учитываться
                              регистр введенных символов

        #Только целые слова#  - при поиске будет произведен
                              поиск только целого слова

    #Искать 16-ричный код#

      Поиск фрагмента, соответствующего шестнадцатеричным кодам,
      набранного в строке #Искать#.
...
имеющегося на диске, то при этом будет создан новый файл.
    ...
    1. Если при открытии нового файла вы указали имя несуществующей папки, то
появится предупреждение: "~Путь к редактируемому файлу не существует~@WarnEditorPath@".

    2. При попытке повторного открытия на редактирование файла, FAR выдаст
предупреждение: "~Повторное открытие файла~@EditorReload@".

    3. При создании нового файла используется WIN кодировка, изменить поведение
можно в меню ~Настройки редактора~@EditorSettings@.
   ...
   #Left#                    Символ влево
   #Ctrl-S#                  Символ влево, но курсор не будет
                           переходить на предыдущую строку при
                           достижении начала строки
   #Right#                   Символ вправо
   #Up#                      Строку вверх
   #Down#                    Строку вниз
  ...
  Операции над блоками
   ...
   #Ctrl-P#                  Копировать блок в текущую позицию курсора
                           (только в режиме постоянных блоков, Буфер
  ...
  Прочие операции
   ...
   #Ctrl-F10#                Позиционировать на текущий файл
                           без выхода.
   #F11#                     Вызвать меню "~Команды внешних модулей~@Plugins@"
   #Alt-F11#                 Показать ~историю редактирования~@HistoryViews@
   #Alt-BS, Ctrl-Z#          Отмена действия
   #Ctrl-L#                  Запретить модификацию редактируемого
                           текста
   #Ctrl-O#                  Показать пользовательский экран
   #Ctrl-Alt-Shift#          Временно показать пользовательский экран
                           (работает до тех пор, пока удерживаем эти
   ...
   #Ctrl-Q#                  Рассматривать следующую комбинацию
                           клавиш как код символа
   #ПравыйCtrl-0..9#         Установить закладку 0..9 в текущей позиции
   #Ctrl-Shift-0..9#         Установить закладку 0..9 в текущей позиции
   #ЛевыйCtrl-0..9#          Перейти к позиции закладки 0..9
   #Shift-Enter#             Вставить в позицию курсора имя текущего
                           файла на панели.
   #Ctrl-Shift-Enter#        Вставить в позицию курсора имя текущего
                           файла на пассивной панели.
   ...
   3. Alt-Код символа на дополнительной клавиатуре вставляет в
      редактируемую строку символ с указанным кодом, причем никаких
      перекодировок в текущую кодировку редактора при этом не
      производится.

   4. Если вы перетаскиваете имя файла из Проводника в редактор
      FAR, то оно вставляется в текущую строку только в
      OEM-кодировке (DOS), текущая кодировка редактора при этом
      не учитывается. Эта особенность будет изменена в следующих
      версиях FAR.
...
является ~панелью файлов~@FilePanel@, ее тип будет изменен на панель файлов.
    ...
    - извлечения дисков из CD-ROM и сменных приводов. В Windows
      NT/2000/XP для операции извлечения диска из ZIP-приводов
...
картридер.
    ...
    Ctrl-1 - тип диска;
    Ctrl-2 - сетевое имя
             (и путь, ассоциированный с SUBST-диском под NT);
    Ctrl-3 - метка диска;
...
работает только под NT-based системах и требует или административных прав, или
разрешения доступа к локальным СD-приводам с помощью политик (для этого надо
запустить из командной строки утилиту #secpol.msc# и установить настройку
'#Локальные политики/Параметры безопасности/Устройства: Разрешить доступ к#
#дисководам компакт-дисков только локальным пользователям#' в значение
'#Разрешить#').
...
"~Параметры~@OptMenu@" (пункт "Раскраска файлов").


@HighlightList
$ #Раскраска файлов: клавиши управления#
    Меню групп ~раскраски файлов~@Highlight@ позволяет манипулировать списком.
...
"Временный" и "Символическая связь" используются только на дисках с файловой
системой NTFS. Причем, атрибуты "Зашифрованный" и "Символическая связь"
используется только в Windows 2000/XP.
...
~программы просмотра~@Viewer@.
  ...
  #файла#                   в недавно просмотренных файлах.
                          ...
                          таблицы символов, использованной
                          при просмотре файла, в случае если
                          эта таблица была установлена пользователем
                          вручную, а также режима просмотра файла
  ...
  #Автоопределение#         Если вы пользуетесь несколькими таблицами
  #таблицы символов#        символов, вы можете включить эту опцию для
                          ~автоопределения таблицы~@DistTable@ просматриваемого
                          файла.
  ...
  #Изначально открывать#    По умолчанию открывать файлы в WIN
  #файлы в WIN кодировке#   кодировке, иначе в DOS кодировке.
...
внешнего и ~встроенного редакторов~@Editor@.
  ...
  #файла#                   в недавно отредактированных файлах.
                          Эта опция также вызывает сохранение
                          таблицы символов, использованной
                          при редактировании файла, в случае если
                          эта таблица была установлена пользователем
                          вручную.
  ...
  #Автоопределение#         Если вы пользуетесь несколькими таблицами
  #таблицы символов#        символов, вы можете включить эту опцию
                          для ~автоопределения таблицы~@DistTable@ редактируемого
                          файла.
  ...
  #Создавать новые файлы#   Создавать новые файлы в WIN кодировке,
  #в WIN кодировке#         иначе файлы создаются в DOS кодировке.
...
$ #Автоопределение таблицы символов#
    Если вы пользуетесь несколькими таблицами символов и установили таблицу с
распределением частот символов для вашего языка (например, для русского языка
должна быть установлена таблица "Addons\\Tables\\Russian\\Dist.Rus.reg"), то
FAR постарается подобрать для просмотра/редактирования наиболее подходящую
кодовую таблицу символов. Заметьте, что корректное определение не
гарантируется, особенно для маленьких или нетипичных текстовых файлов.
...
папки" будет недоступна.
    ...
    Атрибуты "Сжатый", "Зашифрованный", #Неиндексируемый#, #Разреженный# и
#Временный# могут быть изменены только на дисках с файловой системой NTFS.
Причем, атрибут "Зашифрованный" используется только в Windows 2000/XP.
Атрибуты "Сжатый" и "Зашифрованный" являются взаимоисключающими, т.е. можно
выставить только один из них.

    Для ~символических связей~@HardSymLink@ в диалоге отображается информация
об оригинальной папке (только в NTFS/Windows 2000/XP). Если информация об
оригинальной папке недоступна (в частности, для символических связей в сетевых
каталогах), то выводится сообщение "#(нет данных)#".
...
значениями. Доступна для одиночного файлового объекта.
    ...
    Для ~cимволических связей~@HardSymLink@ (поддерживаемых только в
NTFS/Windows 2000/XP) дата и время не выставляются.
...
$ #Фильтр файловой панели#
    С помощью фильтра вы можете определить набор типов файлов, которые будут
показываться в панели файлов.

    Меню фильтра состоит из двух частей. В верхней части расположены
пользовательские фильтры. С помощью клавиш #Ins#, #Del# и #F4# вы можете
добавлять, удалять и редактировать их. Каждый пользовательский фильтр включает
необязательный заголовок и маску файлов, либо несколько ~масок файлов~@FileMasks@,
разделенных запятыми или точкой с запятой.

    В нижней части меню фильтра находятся маски всех файлов, содержащихся в
данный момент в активной панели файлов.

    Для выбора элементов меню фильтров могут использоваться клавиши #пробела#,
'#+#' и '#-#'. Элементы, выбранные с помощью #пробела# или '#+#', помечаются
символом '+'. Если такие элементы присутствуют, то будут показаны только
удовлетворяющие им файлы. Элементы, выбранные с помощью '#-#', помечаются
символом '-', и все удовлетворяющие им файлы будут исключены из панели файлов.
Комбинация #Shift-<Gray Minus># снимет все пометки.
...
буквы режима сортировки в верхнем левом углу панели.
    ...
    Для снятия фильтра с панели необходимо убрать все отметки "+" или "-" с
пользовательских фильтров, с масок файлов текущей панели и нажать #Enter#.
...
длинные описания, владельцы файлов, связи файлов, альтернативный полный. Обычно
...
файла, панель файлов будет отображаться в многоколоночной форме.
    ...
    Атрибуты файла имеют следующие обозначения:
       ...
       #H#         - Скрытый
       #A#         - Архивный
       #L#         - Символическая связь (reparse point)
       #C# или #E#   - Сжатый или Зашифрованный
       #$#         - Разреженный (sparse)
       #T#         - Временный
       #I#         - Неиндексируемый по содержимому

    Порядок отображения атрибутов - RSHALCTI. Символ атрибута "Разреженный"
применяется только для файлов и ставится вместо 'L'. Символ атрибута
"Зашифрованный" ставится вместо 'C', т.к. файл/каталог не могут иметь
одновременно оба атрибута ("Сжатый" и "Зашифрованный"). По умолчанию размер
колонки атрибутов - 6 символов. Для отображения дополнительных атрибутов
'T' и 'I' необходимо явно указать размер колонки в 8 символов.
...
(запятую или точку с запятой), чтобы такая маска не была спутана со списком.
    ...
    В некоторых ситуациях (~поиск~@FindFile@ файлов, ~фильтр операций~@OpFilter@,
~фильтр файловой панели~@Filter@,
~пометка~@SelectFiles@ файлов, ~ассоциации~@FileAssoc@ файлов,
~группы сортировки~@SortGroups@ и ~раскраска~@Highlight@ файлов) можно
...
@CopyFiles
$ #Копирование, перенос, переименование и создание связей#
...
вновь созданную папку #d:\folder2\#.
  ...
  Создание ~связей~@HardSymLink@ файлов и папок                            #Alt-F6#
...
файлам и папкам будут применены наследуемые от родительской папки права.
    ...
    Параметр "Только новые/обновленные файлы" позволит без вопросов
скопировать/переписать новые и обновленные файлы. Под обновленным понимается
файл, дата и время модификации которого больше даты и времени модификации
целевого файла.
...
В Windows 2000 системная функция не используется для случая, когда
файл шифрованный и производится копирование такого файла вне тома.

    Параметр "Копировать содержимое символических связей" позволяет уточнять
~логику~@CopyRule@ работы FAR с ~символическими связями~@HardSymLink@ при
...
одного физического диска), FAR учитывает ~символические связи~@HardSymLink@
Windows 2000/XP.
...
действуют только на текущий сеанс копирования.
    ...
    Для копирования файлов, попадающих под определённые условия, используйте переключатель
#Использовать фильтр#. Настройка условий ~фильтра~@OpFilter@ осуществляется кнопкой #Фильтр#.
Учтите, что если вы копируете каталог с файлами и ни один из них не попадает под условия
фильтра, то пустой каталог в место назначения скопирован не будет.


@CopyRule
$ #Копирование: правила#
  При ~копировании/перемещении~@CopyFiles@ каталогов и
~символических связей~@HardSymLink@ соблюдаются следующие правила.

  #Копирование символической связи#

  Если включена опция "Копировать содержимое символических связей"
  или источник или приемник - сетевые диски, то в приемнике
  создается папка и в нее копируется содержимое связи-источника
  (для вложенных линков рекурсивно).

  Если опция "Копировать содержимое символических связей" выключена,
  и источник и приемник - локальные диски, то в приемнике создается
  символическая связь, ссылающаяся на связь-источник.

  #Перемещение символической связи#

  Если включена опция "Копировать содержимое символических связей"
  или источник или приемник - сетевые диски, то в приемнике
  создается папка и в нее копируется содержимое связи-источника
  (для вложенных линков рекурсивно). Сама связь в источнике
  удаляется.

  Если опция "Копировать содержимое символических связей" выключена,
  и источник и приемник - локальные диски, то связь-источник просто
  переносится. Рекурсивный спуск по дереву не производится.

  #Перемещение каталога, содержащего символические связи#

  Если источник и приемник - локальные диски, то каталог просто
  перемещается.

  Если источник или приемник - сетевые диски, то независимо от
  состояния опции "Копировать содержимое символических связей"
  в приемнике создается папка и в нее копируется содержимое
  связи-источника (для вложенных линков рекурсивно).
  Сама связь в источнике удаляется.


@HardSymLink
$ #Жесткие и символические связи#
    На разделах NTFS также можно создавать #жесткие# (HardLink) связи для
файлов и #символические# (SymLink) для папок с помощью команды #Alt-F6#.

    Жесткие связи применяются для файлов. Символические связи - для папок и
дисков.

    #Жесткие связи#

    Жесткая связь (HardLink) - это просто еще одна запись в папке для данного
файла.

    Когда создается жесткая связь, сам файл не копируется физически, а только
появляется под еще одним именем или в еще одном месте, а его старые имя и
местонахождение остаются нетронутыми. С этого момента жесткая связь
#неотличима# от первоначальной записи в папке. Единственное отличие - то, что
для жесткой связи не создается короткое имя файла, поэтому из программ ДОС она
не видна.
...
пор, пока все жесткие связи, указывающие на него, не будут удалены. Порядок их
удаления значения не имеет. При удалении жесткой связи в корзину количество
связей у файла сохраняется.

    FAR умеет создавать жесткие связи, отображать количество жестких связей для
каждого файла в отдельной колонке (по умолчанию это последняя колонка в 9-м
режиме панелей), а также сортировать файлы по количеству жестких связей.

    Жесткие связи поддерживаются в NTFS, начиная с NT 4.0. Создавать жесткие
связи можно только на том же диске, на котором расположен исходный файл.
...
"directory junctions" - технология, позволяющая отображать любые локальные
папки на любые другие локальные папки. Например, если папка D:\\SYMLINK
указывает на C:\\WINNT\\SYSTEM32 в качестве своей цели, то программа,
обращающаяся к D:\\SYMLINK\\DRIVERS, будет на самом деле обращаться к
C:\\WINNT\\SYSTEM32\\DRIVERS.

    FAR умеет создавать символические связи только на локальных дисках,
файловая система которых поддерживает эту возможность (Windows 2000/XP/NTFS 5.0).
В отличие от жестких связей, символические связи не обязательно должны
указывать на тот же диск.

    Напрямую делать символические связи на папки CD-ROM нельзя, но можно это
ограничение обойти, смонтировав CD-ROM диск на папку NTFS-раздела.
...
$ #Ошибка: копирование/перенос "в самого себя"#
    ...
    Данная ошибка также возникает в Windows 2000/XP, если имеются две папки,
одна из которых является ~символической связью~@HardSymLink@ на другую.
...
потоки. В частности, потоки использует Windows Explorer в Windows 2000/XP для
хранения дополнительных свойств файла. Файловые системы FAT/FAT32 не
поддерживают потоки.
...
предупреждение и файл копируется в новое место нешифрованным.
    ...
    В Windows 2000 для копирования шифрованного файла за пределы
исходного диска будет использован внутренний механизм копирования,
независимо от того, включен ли параметр "Использовать системную функцию
копирования" или нет.
...
существует файл "file2" и определена переменная среды "variable".
    ...
    1. Если синтаксис команд отличается от указанных, то
...
дочерним процессам:
    ...
    #FARUSER#            имя пользователя, указанное в параметре /u
...
#Ctrl-F5#, #MsWheelUp# и #MsWheelDown# с комбинацией #Ctrl#, #Shift# и #Alt#)
