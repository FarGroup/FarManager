...
DWORD CountRecords;
Record {
    RECHEADER Header;  // заголовок
    // RECHEADER Rec[0];   // количеством Header.RecordsCount
} * CountRecords
*/
...
struct FAULTCODERECORD{   // кусок кода
  ...
  WORD  SizeRec;          // Размер данных
  DWORD ExceptionAddress; // адрес исключения
  BYTE  Code[64];         // первые 32 байта перед тем местом, где был трап
                          // 32-й байт - это и есть то самое место!
                          // эти 64 байта могут быть заполнены 0, если не
