...
system "svn export -q --force --username=fargroup --password=secret svn://localhost/svn/enc/tools/trunk/inet/ ".$dest_dr_inet."/inet";
...
sub mk_inet_lng
{
  ...
  print " -- preparing ".$dr1." --\n";
  system "svn export -q --force --username=fargroup --password=secret svn://localhost/svn/enc/meta_".$dr2."/trunk ".$dest_dr_inet."/meta.".$dr1;
