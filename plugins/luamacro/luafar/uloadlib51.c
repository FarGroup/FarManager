...
#undef  LUA_PATHSEP
#define LUA_PATHSEP   L";"

#undef  LUA_PATH_MARK
#define LUA_PATH_MARK L"?"

#undef  LUA_EXECDIR
#define LUA_EXECDIR   L"!"

#undef  LUA_IGMARK
#define LUA_IGMARK    L"-"

#undef  LUA_PATH
#define LUA_PATH      L"LUA_PATH"

#undef  LUA_CPATH
#define LUA_CPATH     L"LUA_CPATH"

#undef  LUA_LDIR
#define LUA_LDIR      L"!\\lua\\"
...
#define LUA_PATH_DEFAULT  \
	L".\\?.lua;"  LUA_LDIR L"?.lua;"  LUA_LDIR L"?\\init.lua;" \
	LUA_CDIR L"?.lua;"  LUA_CDIR L"?\\init.lua"

#undef  LUA_CPATH_DEFAULT
#define LUA_CPATH_DEFAULT \
	L".\\?.dll;"  LUA_CDIR L"?.dll;" LUA_CDIR L"loadall.dll"
...
static void *ll_load(lua_State *L, const wchar_t *path);
...
static void setprogdir(lua_State *L)
{
	...
	{
		*lb = L'\0';
		LF_Gsub(L, (const wchar_t*)lua_tostring(L, -1), LUA_EXECDIR, buff);
...
static void *ll_load(lua_State *L, const wchar_t *path)
{
	HINSTANCE lib = LoadLibraryW(path);
...
static int ll_loadfunc(lua_State *L, const wchar_t *path, const char *sym)
{
...
static const wchar_t *pushnexttemplate(lua_State *L, const wchar_t *path)
{
	const wchar_t *l;

	while(*path == *LUA_PATHSEP) path++;   /* skip separators */

	if (*path == L'\0') return NULL;   /* no more templates */

	l = wcschr(path, *LUA_PATHSEP);  /* find next separator */

	if (l == NULL) l = path + wcslen(path);

	lua_pushlstring(L, (const char*)path, sizeof(wchar_t) *(l - path));   /* template */
...
static int loader_C(lua_State *L)
{
	const char *funcname;
	const char *name = luaL_checkstring(L, 1);
	const wchar_t *filename = findfile(L, name, "cpath");
