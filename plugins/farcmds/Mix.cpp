...
/*
	«аменить в строке Str Count вхождений подстроки FindStr на подстроку ReplStr
	≈сли Count < 0 - замен€ть "до полной победы"
...
/*
	...
	NewCmdStr и NewCmdPar после использовани€ удалить
*/
int PartCmdLine(const wchar_t *CmdStr,wchar_t **NewCmdStr,wchar_t **NewCmdPar)
{
	...
	if (Temp)
	{
		...
		// –азделим собственно команду дл€ исполнени€ и параметры.
		...
		if (NewCmdPar && ParPtr) // ћы нашли параметры и отдел€ем мух от котлет
		{
